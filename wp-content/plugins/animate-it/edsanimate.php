...
class EDS_Animate {
	...
	public function __construct( $file_loc ) {	
		...
		// Function to add animate it block
		add_action( 'init', array( $this, 'register_animateit_block' ) );
	...
	function register_animateit_block() {
		...
		wp_register_style(
			'edsanimate-block-style',
			plugins_url( 'assets/css/block-style.css', __FILE__ ),
			array( ),
			filemtime( plugin_dir_path( __FILE__ ) . 'assets/css/block-style.css' )
		);

		register_block_type( 'eds/animate-it-block', array(
			'style' => 'edsanimate-block-style',
			'editor_style' => 'edsanimate-block-editor-style',
			'editor_script' => 'edsanimate-block-script',
		) );

		if ( function_exists( 'wp_set_script_translations' ) ) {
			wp_set_script_translations( 'edsanimate-block-script', 'eds-animate', plugin_dir_path( __FILE__ ) . 'lang/' );
		}
	}
}
